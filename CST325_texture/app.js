...
function initGL(canvas) {
    ...
    try {
        ...
        // todo enable depth test (z-buffering) - DONE
...
function updateAndRender() {
    ...
    // todo
    //   1. enable blending - DONE
    gl.enable(gl.BLEND);

    //   2. set blend mode source to gl.SRC_ALPHA and destination to gl.ONE_MINUS_SRC_ALPHA - DONE
    gl.blendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA);

    // sort sphereGeometryList
    const sphereOrder = sphereGeometryList.sort(function(a,b) {
        const firstSphere = camera.getPosition().subtract(a.getPosition()).length();
        const secondSphere = camera.getPosition().subtract(b.getPosition()).length();
        return firstSphere > secondSphere ? -1 : 1;
    });

// uncomment when directed by guide
    for (var i = 0; i < sphereOrder.length; ++i) {
        sphereOrder[i].render(camera, projectionMatrix, textureShaderProgram);
    };


    // todo - disable blending - DONE
    gl.disable(gl.BLEND);
}
